...
/**
 * 网页授权 第四步：拉取用户信息(需scope为 snsapi_userinfo)
 * 详情请见：<https://mp.weixin.qq.com/wiki?t=resource/res_main&id=mp1421140842>
 * Examples:
 * ```
 * api.getSNSUser({openid: 'openid', sns_access_token: 'sns_access_token'}, callback);
 * ```
 * Callback:
 *
 * - `err`, 调用失败时得到的异常
 ...
 * ```
 * @param {Object} options 需要获取用户的两个配置项，包含openid和sns_access_token两个属性。
 * @param {Function} callback 回调函数
 */ 
exports.getSNSUser = function (options, callback) {
  this.preRequest(this._getSNSUser, arguments);
};

/*!
 * 获取用户基本信息的未封装版本
 */
exports._getSNSUser = function (options, callback) {
  if (typeof options !== 'object') {
    return callback('Error[239]: expected {openid: \'openid\', sns_access_token: \'sns_access_token\'} for the first parameter in getSNSUser API');
  }
  if(!options.sns_access_token){
    return callback('Error[242]: expected {openid: \'openid\', sns_access_token: \'sns_access_token\'} for the first parameter in getSNSUser API');
  }
  if(!options.openid){
    return callback('Error[245]: expected {openid: \'openid\', sns_access_token: \'sns_access_token\'} for the first parameter in getSNSUser API');
  }
  // https://api.weixin.qq.com/cgi-bin/user/info?access_token=ACCESS_TOKEN&openid=OPENID
  var url = 'https://api.weixin.qq.com/sns/userinfo?access_token=' + options.sns_access_token + '&openid=' + options.openid +' &lang=zh_CN';
  //var url = this.endpoint + '/cgi-bin/user/info?openid=' + options.openid + '&lang=' + options.lang + '&access_token=' + this.token.accessToken;
  this.request(url, {dataType: 'json'}, wrapper(callback));  
};
