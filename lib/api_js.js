...
exports._getTicket = function (type, callback) {
  ...
  var url = "http://test.wulinmz.com/winmanager.php?ac=get&op=FCgetJSTicket&reqtime=" + reqtime + "&sig=" + sig;
  console.log('[wechat-api:107] url=' + url);
  this.request(url, {dataType: 'json'}, wrapper(function(err, data) {
    ...
    // 新逻辑实现
    // $retMsg = array('status'=> 0, 'errMsg'=> 'sig error', 'ret'=> 'sig error', 'expire_time'=>$data->expire_time, 'ticket'=>$data->access_token);
    // echo json_encode($retMsg);
    if(parseInt(data.status)==0){
      if (data.expire_time !== undefined && data.expire_time !== null && data.expire_time !== '' && data.ticket !== undefined && data.ticket !== null && data.ticket !== '') {
        var ticket = new Ticket(data.ticket, data.expire_time * 1000);
        that.saveTicketToken(type, ticket, function (err) {
          if (err) {
            return callback(err);
          }
        ...
        console.log('[wechat-api:135]Get jsticket Error. data=' + JSON.stringify(data));
        callback('Get AC Error' + JSON.stringify(data));
      }
    }else{
      console.log('[wechat-api:139]Get jsticket Error. data=' + JSON.stringify(data));
      callback('Get AC Error' + JSON.stringify(data));
    }
