...
API.prototype.getAccessToken = function (callback) {
  ...
  var reqtime = parseInt((new Date().getTime()) / 1000);
  var sig = md5('wlmz2017-' + reqtime);
  var url = "http://test.wulinmz.com/winmanager.php?ac=get&op=FCgetAccessToken&reqtime=" + reqtime + "&sig=" + sig;
  //console.log('[wechat-api:201] url=' + url);
  this.request(url, {dataType: 'json'}, wrapper(function (err, data) {
    ...
    // 新逻辑实现
    // $retMsg = array('status'=> 0, 'errMsg'=> 'sig error', 'ret'=> 'sig error', 'expire_time'=>$data->expire_time, 'access_token'=>$data->access_token);
    // echo json_encode($retMsg);
    if(parseInt(data.status)==0){
      if (data.expire_time !== undefined && data.expire_time !== null && data.expire_time !== '' && data.access_token !== undefined && data.access_token !== null && data.access_token !== '') {
        var token = AccessToken(data.access_token, data.expire_time * 1000);
        that.saveToken(token, function (err) {
            if (err) {
              return callback(err);
            }
        ...
        //console.log('[wechat-api:223]Get Ac Error. data=' + JSON.stringify(data));
        callback('Get AC Error' + JSON.stringify(data));
      }
    }else{
      //console.log('[wechat-api:222]Get Ac Error. data=' + JSON.stringify(data));
      callback('Get AC Error' + JSON.stringify(data));
    }
  }));
  return this;
...
API.prototype.preRequest = function (method, args, retryed) {
  ...
  that.getToken(function (err, token) {
    ...
    //console.log('[wechat-api:215]' + JSON.stringify(token));
    //console.log('[wechat-api:216]' + (AccessToken(token.accessToken, token.expireTime)).isValid() );
    var accessToken;
    // 有token并且token有效直接调用
